...
| `DetectionService` | — | `RegionDetectionState` (split 2/4 per-region unions only, spec §5a — no per-page cache, since crop_rects has no per-page anchor to read one back out for) | `detect_content` (Auto-detect, split=1 or split=2/4 via `_run_detect_split`/`_write_split_crop_rects`) + union aggregation + committed-crop refresh | adapter, `PageRasterPipeline`, `PageIndexMap`, live crop-state reads |
