...
| `DetectionService` | — | `RegionDetectionState` (split 2/4 per-region cache/unions, spec §5a) | `detect_content` (Auto-detect, split=1 or split=2/4 via `_run_detect_split`/`_write_split_crop_rects`) + union aggregation + committed-crop refresh | adapter, `PageRasterPipeline`, `PageIndexMap`, live crop-state reads |
